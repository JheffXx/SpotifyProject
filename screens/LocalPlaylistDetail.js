...
// LocalPlaylistDetail.js (Refactorizado con principios SOLID)
import React, { useState } from 'react';
import { SafeAreaView, StyleSheet, Text } from 'react-native';
import { useNavigation } from '@react-navigation/native';
import Header from './Refactorizado/Header';
import PlaylistControls from './Refactorizado/PlaylistControls';
import SongList from './Refactorizado/SongList';
...
const LocalPlaylistDetail = ({ route }) => {
    ...
    const skipTrack = () => setCurrentTrack(prev => (prev + 1) % songs.length);
    const previousTrack = () => setCurrentTrack(prev => (prev > 0 ? prev - 1 : songs.length - 1));
    const playSong = (index) => {
        setCurrentTrack(index);
        setIsPlaying(true);
    };
    ...
    return (
        <SafeAreaView style={styles.container}>
            <Header playlistName={playlistName} onBack={() => navigation.goBack()} />
            ...
            <Text style={styles.status}>
                {isPlaying
                    ? `Reproduciendo: ${songs[currentTrack]?.name}`
                    : `Canción pausada - ${songs[currentTrack]?.name}`}
            </Text>
